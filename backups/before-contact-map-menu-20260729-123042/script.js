...
if (musicPlayer && backgroundAudio && musicToggle) {
  ...
  backgroundAudio.addEventListener("play", () => {
    ...
    if (musicIcon) musicIcon.textContent = "II";
  });
...
const pannellumViewerInstances = new WeakMap();

function createPannellumViewer(element) {
  if (!element || !window.pannellum || !element.dataset.panorama || pannellumViewerInstances.has(element)) return null;

  const viewer = window.pannellum.viewer(element, {
    type: "equirectangular",
    panorama: element.dataset.panorama,
    title: element.dataset.pannellumTitle || "Recorrido 360",
    autoLoad: true,
    showControls: true,
    showZoomCtrl: true,
    showFullscreenCtrl: true,
    compass: false,
    hfov: Number(element.dataset.hfov || 100),
    minHfov: 55,
    maxHfov: 120,
    pitch: Number(element.dataset.pitch || 0),
    yaw: Number(element.dataset.yaw || 0),
    mouseZoom: true,
    draggable: true
  });

  pannellumViewerInstances.set(element, viewer);
  element.classList.add("pannellum-ready");
  return viewer;
}

function setPannellumPanorama(element, panorama) {
  if (!element || !panorama) return;
  element.dataset.panorama = panorama;

  const currentViewer = pannellumViewerInstances.get(element);
  if (currentViewer && typeof currentViewer.destroy === "function") {
    try {
      currentViewer.destroy();
    } catch (error) {
      // Pannellum puede lanzar si el visor se destruye mientras carga; se recrea limpio abajo.
    }
  }

  pannellumViewerInstances.delete(element);
  element.classList.remove("pannellum-ready");
  element.innerHTML = "";
  createPannellumViewer(element);
}

function initPannellumViewers() {
  const viewers = document.querySelectorAll(".pannellum-viewer[data-panorama]");
  if (!viewers.length) return;

  if (!("IntersectionObserver" in window)) {
    viewers.forEach(createPannellumViewer);
    return;
  }
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      createPannellumViewer(entry.target);
